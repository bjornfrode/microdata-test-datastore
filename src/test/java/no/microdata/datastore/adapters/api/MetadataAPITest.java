...
class MetadataAPITest {
    ...
    @Test
    public void testGetDataStructuresWithMalformedUrl() throws Exception {
        var expectedDataStructures = MetadataAPIFixture.dataStructures();
        MetadataQuery query = new MetadataQuery(Map.of(
                "names", List.of("FNR", "AKT_ARBAP"),
                "languages", "no",
        ...
        mockMvc.perform(
                get("/metadata/DOESNOTEXIST/data-structures?revisionId={revisionId}&names={names}",
                        query.getVersion(), names)
                        .contentType(MediaType.APPLICATION_JSON)
                        .header(Constants.X_REQUEST_ID, query.getRequestId())
                        .header(Constants.ACCEPT_LANGUAGE, query.getLanguages())
                        .header(Constants.ACCEPT, Constants.ACCEPT_JSON))
        ...
        verifyNoMoreInteractions(dataStructureService);
    }

    @Test
    public void testGetDatastructureVersion() throws Exception {
        var requestId = "test-123";
        var datastoreVersion = "4.1.0.0";
        var name = "INNTEKT_BANKINNSK";
        var expectedDataStructureVersion = Map.of(
                "datastructureName",  name,
                "datastructureVersion", "4.0.0.0"
        );

        when(dataStoreService.findDatastructureVersion(
                requestId, name, VersionUtils.toThreeLabelsIfNotDraft(datastoreVersion)))
                .thenReturn(expectedDataStructureVersion);

        mockMvc.perform(
                get("/metadata/data-structure/version?datastructureName={name}&dataStoreVersion={datastoreVersion}",
                ...
                .andExpect(status().isOk())
                .andExpect(content().contentType(CONTENT_TYPE_JSON))
                .andExpect(content().json(new ObjectMapper().writeValueAsString(expectedDataStructureVersion)))
                .andDo(document("getDatastructureVersion",
                        Preprocessors.preprocessResponse(Preprocessors.prettyPrint()),
                        requestParameters(
                                parameterWithName("datastructureName").description(DATASTUCTURE_NAME_DESCRIPTION),
                                parameterWithName("dataStoreVersion").description(DATASTORE_VERSION_DESCRIPTION)
                        ),
                        requestHeaders(
                                HeaderDocumentation.headerWithName(Constants.CONTENT_TYPE).description(CONTENT_TYPE_DESCRIPTION),
                                HeaderDocumentation.headerWithName(Constants.X_REQUEST_ID).description(REQUESTID_DESCRIPTION),
                        ...
                        responseHeaders(
                                HeaderDocumentation.headerWithName(Constants.CONTENT_TYPE).description(CONTENT_TYPE_DESCRIPTION)),
                        responseFields(
                                subsectionWithPath("datastructureName").description("The name of the data structure"),
                                subsectionWithPath("datastructureVersion").description("The data structure version")
                        )));

        verify(dataStoreService).findDatastructureVersion(
                requestId, name, VersionUtils.toThreeLabelsIfNotDraft(datastoreVersion));
    }
